...
- 2026-09-17 12:45 `operators/gemm`：删除独立 v3 变体，改为同一 v3 内核的 TM1 / TM8 对照
  - 删除独立内核文件、其注册项和非对齐专项；默认入口注册 `sgemm_v3_TM1<32,32,32,1>`（`block=(1024,1)`）和 `sgemm_v3_TM8<64,64,8,8>`（`block=(512,1)`）。
  - 验证：Release 构建通过；默认入口 6/6 PASS。
  - 扫描产物：`--bench` 36 点全部成功，CSV / PNG 位于 `operators/gemm/bench/20260917124531/`；512³ 上 TM1 为 1037.0 GFLOP/s，TM8 为 2693.9 GFLOP/s。
