...
- 计时：开发档 1 次预热 + 100 次迭代；严格档采样量按设备算力分档（基准机 RTX 4060
  Laptop，24 SM → 预热 100 次 + 21 组 × 1000 次；更强 GPU 按
  `tier = clamp(round(SM/24), 1, 10)` 放大，见 `docs/benchmark-methodology.md` §3.2 与
  `common/include/operator_common/BenchConfig.h`），报 **中位数**与 P5/P95。
...
9. **计时口径**：中位数为主指标，P5/P95 反映波动；严格档采样量按设备 SM 数分档（基准机
   RTX 4060 Laptop = 100 预热 + 21×1000，更强 GPU 按 `tier` 放大，见 §1）出的数字才进
   结论表，开发档只做快速冒烟。注意工作集小于 L2 时“带宽”是 L2 命中带宽。
