...
| CUDA | v3 | 每线程多元素展开 + `float4` 向量化（需对齐约束与尾列处理） | 规划中 |
...
- **v3 向量化（规划中）**：在块内归约前用 `float4` 连续加载 + 每线程多元素
  展开（需对齐约束与尾列处理），减少指令与访存事务。
...
测试入口 `src/main.cu` 将 `reduce_v0` / `reduce_v1` / `reduce_v2` 注册给同一
测试驱动，覆盖三类场景（每组场景三个内核各跑一遍）：
...
| 开关 | 默认 | 作用 |
| --- | --- | --- |
| `enable_boundary` | `false` | 是否执行“边界条件”与“异常 / 健壮性”场景；关闭时只跑正常流程（6 项），开启后为全量回归（27 项） |
