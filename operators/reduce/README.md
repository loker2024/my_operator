...
| CUDA | v5 | v4 + `float4` 向量化加载（需对齐约束与尾列处理） | 规划中 |
...
- **v4 末 warp 展开（本轮完成）**：加载与覆盖口径同 v3（每 block 覆盖
  ...
  被测内核表。
- **v5 float4 向量化（规划中）**：在块内归约前用 `float4` 连续加载 + warp shuffle
...
- 默认：`n = 2^20`（fp32，约 4 MiB 输入；v0/v1/v2 的 `grid = n/block = 4096`，
  v3/v4 每线程 2 元素 → `grid = n/(2*block) = 2048`）。
...
└── src/        # CUDA 实现（v0…v4）
    ...
    └── main.cu                  # 执行入口：注册 v0…v4，运行测试
```

> 注：`triton/`（第二阶段 Triton 实现）与 `notes/`（学习笔记）属规划目录，尚未创建。

测试入口 `src/main.cu` 将 `reduce_v0` … `reduce_v4` 注册给同一测试驱动，
覆盖三类场景（每组场景五个内核各跑一遍）：
...
| 开关 | 默认 | 作用 |
| --- | --- | --- |
| `enable_boundary` | `false` | 是否执行“边界条件”与“异常 / 健壮性”场景；关闭时只跑正常流程（10 项），开启后为全量回归（45 项） |
