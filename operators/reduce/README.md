...
| CUDA | v4 | v3 + `float4` 向量化加载与 warp shuffle（需对齐约束与尾列处理） | 规划中 |
...
- **v4 向量化（规划中）**：在块内归约前用 `float4` 连续加载 + warp shuffle
...
- 默认：`n = 2^20`（fp32，约 4 MiB 输入；v0/v1/v2 的 `grid = n/block = 4096`，
  v3 每线程 2 元素 → `grid = n/(2*block) = 2048`）。
...
测试入口 `src/main.cu` 将 `reduce_v0` … `reduce_v3` 注册给同一测试驱动，
覆盖三类场景（每组场景四个内核各跑一遍）：
...
| 开关 | 默认 | 作用 |
| --- | --- | --- |
| `enable_boundary` | `false` | 是否执行“边界条件”与“异常 / 健壮性”场景；关闭时只跑正常流程（8 项），开启后为全量回归（36 项） |
