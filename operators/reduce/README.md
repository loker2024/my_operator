...
固定覆盖两组正常场景（每组场景八个内核各跑一遍）：

- **正常流程**：大规模对齐（`n=2^20`）、尾部非对齐（`n=2^20+1000`）；

入口固定使用快速采样口径（1 次预热 + 100 次迭代），每次运行执行 16 项测试。
...
下列严格基准数据为历史实测记录，采样口径见 `docs/benchmark-methodology.md`。
...
此后严格档下调为 100 预热 + 21 组 × 1000 次（见 `docs/benchmark-methodology.md` §3.2）。
