...
// TILE_SIZE×TILE_SIZE 的输出 tile，每线程一个输出元素；不使用共享内存与寄存器分块，
// k 方向每步直接读全局内存 —— warp 内 B 的读取按列连续合并，A 的读取同址广播。
// 模板内核内联定义在头文件（-rdc=false 下跨翻译单元引用 __global__ 模板特化已被 nvcc
// 弃用），故 main.cu 直接包含本文件并以 TILE_SIZE=32 显式实例化、注册进被测内核表。
// ============================================================================
...
//   0 B。blockDim.x 必须给满 TILE_SIZE² —— 线性映射把 threadIdx.x / TILE_SIZE 当作 tile 内
...
__global__ void sgemm_v1(const float* A, const float* B, float* C, const uint M, const uint N,
                         const uint K) {
	// 线性线程 → tile 内行列：前 TILE_SIZE 个线程填第 0 行，依此类推。
	const int cRow = blockIdx.x * TILE_SIZE + threadIdx.x / TILE_SIZE;
	const int cCol = blockIdx.y * TILE_SIZE + threadIdx.x % TILE_SIZE;
