...
- **v1 全局内存访问合并**：保留 `sgemm_v1<32>` 的原始模板实现；用 `TILE_SIZE² = 1024` 个线性线程覆盖一个 `32×32` 输出 tile，不使用共享内存。启动约束为 `block=(1024,1)`、`grid=(ceil(M/32),ceil(N/32))`、动态共享内存 0 B —— 线程数不足 `TILE_SIZE²` 时每 block 只写出 tile 首行（静默算错的坑，见「结论记录」）。
