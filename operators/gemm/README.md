...
- 默认快速回归：仅 `512×512×512`。输出顺序固定为 cuBLAS 对照、v0、v1、v2；每项使用 1 次预热与 100 次计时迭代、CPU double 参考和 GPU 逐元素验证。v0 使用 `block=(16,16)`；v1 与 v2 均使用 `block=(1024,1)`，以 `TILE_SIZE=32` 覆盖 `32×32` 输出 tile，动态共享内存为 0 B（v2 的共享内存为静态分配，8 KiB）。
