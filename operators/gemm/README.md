...
| CUDA | v3 | `sgemm_v3<32,32,32,8>`：共享内存 K-tile + 每线程连续计算 8 个输出行 | 完成（已接入测试） |
...
- **v3 共享内存分块 + 寄存器行分块**：`sgemm_v3<32,32,32,8>` 用 128 个线性线程覆盖一个 `32×32` 输出 tile，每线程计算同一列连续 8 行的 8 个结果。全体线程以步长 128 装载 A / B 的两个 `32×32` tile 到静态共享内存；K 循环中每线程从 B 的当前列读取一个值，并与 A 的 8 行相乘累加到寄存器数组。启动为 `block=(128,1)`、`grid=(ceil(N/32),ceil(M/32))`、动态共享内存 0 B（静态共享内存 8 KiB）；M/N/K 尾块分别以加载补 0 与写回保护处理。
- 后续（可选）：考虑仿射对齐 `A^T` 版本、split-K 或更细粒度优化，视学习节奏再定。

## 测试与参考规模

- 默认快速回归：仅 `512×512×512`。输出顺序固定为 cuBLAS 对照、v0、v1、v2、v3；每项使用 1 次预热与 100 次计时迭代、CPU double 参考和 GPU 逐元素验证。v0 使用 `block=(16,16)`；v1 与 v2 均使用 `block=(1024,1)`，以 `TILE_SIZE=32` 覆盖 `32×32` 输出 tile；v3 使用 `block=(128,1)`，每线程计算连续 8 行。动态共享内存均为 0 B（v2/v3 的共享内存为静态分配，各 8 KiB）。
- v3 边界专项：`./build/operators/gemm/gemm --v3-boundary` 只运行 `510×514×518`，覆盖 M/N/K 均不整除 32 的尾块。
...
### 多尺寸性能曲线（2026-09-16 20:15:34 采样，含 v3）

![GEMM 内核性能曲线（含 v3）](bench/20260916201534/gemm_bench.png)

`--bench` 默认参数、`--sizes 128…4096`，同机同构建，单位 GFLOP/s（图与 `bench/20260916201534/gemm_bench.csv` 同属一次扫描）：

| 版本 | 128 | 256 | 512 | 1024 | 2048 | 4096 |
| --- | --- | --- | --- | --- | --- | --- |
| cuBLAS | 200.1 | 1681.3 | 3724.2 | 4893.0 | 5339.4 | 5322.0 |
| v3 | 141.1 | 1335.9 | 2170.1 | 2095.2 | 1761.6 | 1369.0 |
| v2 | 228.4 | 678.6 | 1123.5 | 1169.6 | 1184.7 | 955.2 |
| v1 | 260.3 | 623.6 | 617.6 | 612.2 | 662.9 | 562.4 |
| v0 | 99.6 | 166.2 | 216.2 | 219.6 | 223.7 | 182.7 |

- v3 在 256³ 之后超过 v2：512³ / 1024³ / 2048³ / 4096³ 分别为 v2 的 1.93× / 1.79× / 1.49× / 1.43×；128³ 受启动与小规模开销影响，低于 v2。
- 本轮 4096³ 的每组迭代数为 1，仅用于版本趋势观察，不构成严格性能结论。
