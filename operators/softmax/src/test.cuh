...
//   以函数指针接收任意符合 SoftmaxKernel 签名的 softmax 内核，使各版本共用同一套
...
//                         任何元素、直接通过）
//   grid / block          启动网格 / 每 block 线程数（均 >= 1）。由调用方按被测
//                         内核的行映射给出（见 main.cu 的 RowMap / GridFor）
//   smem_bytes            每 block 动态共享内存字节数，同样由调用方按行映射给出
//                         （见 main.cu 的 SmemFor）：无共享内存的内核为 0，整行
