...
// 每 block 线程数（默认 256）：v0/online-v0 用它铺满行号，v1/v2/v3/v4/v5 用作每行
// 的协作线程数。256 同时满足 v1 的"2 的幂"（折半归约）与 v2/v3/v4/v5 的"32 的倍数"
...
enum class RowMap {
	...
	kBlockPerRowShuffle,  // v2/v3：每行一个 block + 两级 warp shuffle 归约，
	                      //     grid = rows，无动态共享内存（内部仅静态 __shared__
	                      //     中转）。v3 的 float4 向量化只影响行内访问，启动配置同 v2
...
const KernelEntry kKernels[] = {
    ...
    {"softmax_v5 (每行一个 block, 全局读2遍 float4, smem 存 exp)", softmax_v5,
     RowMap::kBlockPerRowRowCache},
    {"online_softmax_v0 (每线程处理一行, 单趟在线归约)", online_softmax_v0,
...
// 恰满载 / 超 1 / 末 block 仅余 1 行），列宽在 v1/v2/v3/v4/v5 的"行内协作"边界
// 附近（差 1 / 恰满载 / 超 1 / 第 2 轮仅余 1 列，以及 warp 边界附近的 31/32/33 ——
// v2/v3/v4/v5 warp shuffle 归约的关键路径：仅前几个 warp 持有数据、其余 warp 以
...
// 按行映射方式求每 block 的动态共享内存（字节数，见 softmax.cuh）；除 v4/v5
