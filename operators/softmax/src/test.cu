...
bool test_softmax_kernel(SoftmaxKernel kernel, const char* kernel_name, int rows, int cols,
	...
	if (rows < 0 || cols < 0 || grid < 1 || block < 1) {
		std::printf(
		    "[%s] 非法参数: rows=%d, cols=%d, grid=%d, block=%d "
	...
	const size_t in_bytes = static_cast<std::size_t>((count > 0 ? count : 1)) * sizeof(float);
	...
	// 版本；args 中需放与形参 const 修饰严格匹配的指针。grid/block/smem_bytes 由调用
	// 方按被测内核的行映射给出（见 main.cu 的 RowMap / GridFor / SmemFor）。
	...
	// （逻辑数据量，每元素计 1 读 1 写；各版本实际读行次数 / 访问模式各异，低效会
	// 直接反映为更低的“有效带宽”，见 README）。
