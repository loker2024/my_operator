...
__global__ void softmax_v3(float* input, float* output, int M, int N) {
	...
	for (int i = tid; i < N4; i += blockDim.x) {
		float4 data = x4[i];
		local_sum += expf(data.x - max_val) + expf(data.y - max_val) + expf(data.z - max_val) +
		             expf(data.w - max_val);
	}
	for (int i = N4 * 4 + tid; i < N; i += blockDim.x) {
...
int main() {
	...
	// (base) loker1@localhost:~/my_operator/operators/softmax/src$ nvcc -o softmax_v3
	// softmax_v3_test.cu nvcc warning : Support for offline compilation for architectures prior to
	// '<compute/sm/lto>_75' will be removed in a future release (Use -Wno-deprecated-gpu-targets to
	// suppress warning). (base) loker1@localhost:~/my_operator/operators/softmax/src$ ./softmax_v3
