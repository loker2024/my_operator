...
- **构建须在 WSL（Ubuntu-24.04）内进行**：CUDA Toolkit、gcc / nvcc 与构建缓存都按 Linux 路径生成，Windows 侧 CMake 会被顶层 `CMakeLists.txt` 的 `CMAKE_HOST_WIN32` 守卫直接拒绝。
...
- Python 3 + matplotlib：仅 `scripts/` 下的绘图脚本需要；统一使用项目根目录的 `.venv/`，配置见 [`docs/python-environment.md`](docs/python-environment.md)。
