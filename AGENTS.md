...
| Softmax | 进行中 | v0/v1/v2/v3/v4/v5 与 online-v0/v1/v2/v3/v3_false 完成，全量回归 220 项通过 |
...
- **注释的硬性要求**（三个高频缺陷，改完代码逐条自查）：
  1. **不缺**：新增 `.cu` / `.cuh` 一律带注释，**禁止提交裸文件**。必备项 = 文件头模块说明 + 每个函数的「作用 / 参数 / 返回值 / **启动约束**（grid 怎么算、blockDim 取值限制、动态共享内存字节数）/ 注意事项（越界空转、空输入、对齐要求、误差与性能代价）」。CUDA 内核尤其要写清启动约束 —— 否则 `main.cu` 无从选择行映射与 `smem_bytes`。
  2. **不冗余**：注释精简，够用即止。**推导过程、公式来源、版本对比、性能结论一律写入算子 README / notes，不堆在源码**；声明处写契约与启动约束，实现处只写「结构要点 + 指向 `.cuh` / README」，同一信息不重复两遍。宁可少写，不要长篇。
  3. **不失真**：注释与代码同时改 —— 代码改了注释没改即视为缺陷；**代码与文档（README / CHANGELOG）冲突时以实际代码为准**，先按代码修正启动配置与注册，再回头同步文档；禁止照抄文档描述或凭印象写出与实现不符的注释（如把「每线程处理一行」写成「每行一个 block + warp shuffle 归约」）。
