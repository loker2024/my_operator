...
static void SubmitChunk(int off, int chunk,
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_a + off, h_a + off, chunk_bytes,
                               cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(d_b + off, h_b + off, chunk_bytes,
                               cudaMemcpyHostToDevice, stream));
    VecAddKernel<<<blocks, threads, 0, stream>>>(d_a + off, d_b + off, d_c + off,
                                                 chunk);
    CUDA_CHECK(cudaMemcpyAsync(h_c + off, d_c + off, chunk_bytes,
...
static float RunPipelineOnce(int n_chunks, int n,
                             const float* h_a, const float* h_b, float* h_c,
                             float* d_a, float* d_b, float* d_c,
                             int blocks, int threads,
                             cudaStream_t* streams,
                             cudaEvent_t* starts, cudaEvent_t* stops) {
    ...
    for (int s = 0; s < n_chunks; ++s) {
        ...
        SubmitChunk(off, chunk, h_a, h_b, h_c, d_a, d_b, d_c,
                    blocks, threads, streams[s]);
...
static float RunPipelineMedian(int n_chunks, int iters, int n,
                               const float* h_a, const float* h_b, float* h_c,
                               float* d_a, float* d_b, float* d_c,
                               int blocks, int threads) {
    ...
    // 预热一轮，消除冷启动影响
    RunPipelineOnce(n_chunks, n, h_a, h_b, h_c, d_a, d_b, d_c,
                    blocks, threads, streams.data(), starts.data(), stops.data());

    std::vector<float> times;
    times.reserve(iters);
    for (int it = 0; it < iters; ++it) {
        times.push_back(RunPipelineOnce(n_chunks, n, h_a, h_b, h_c, d_a, d_b, d_c,
                                        blocks, threads, streams.data(),
...
int main() {
    ...
    printf("grid-stride grid : %d blocks x %d threads (SM=%d x 8 waves)\n\n",
           blocks, threads, prop.multiProcessorCount);
    ...
    for (int n_chunks : stream_configs) {
        const float ms = RunPipelineMedian(n_chunks, gpu_iters, n,
    ...
    const int best_streams = stream_configs[std::min_element(pipeline_ms.begin(),
                                     pipeline_ms.end()) - pipeline_ms.begin()];
